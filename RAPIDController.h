 ...
 * PIDOutput
 */
class RAPIDController : public LiveWindowSendable, public Controller, public ITableListener
{
public:
	RAPIDController(float p, float i, float d,
					PIDSource *source, PIDOutput *output,
					float period = 0.05);
	RAPIDController(float p, float i, float d, float f,
					PIDSource *source, PIDOutput *output,
					float period = 0.05);
	virtual ~RAPIDController();
	virtual float Get();
	virtual void SetContinuous(bool continuous = true);
	virtual void SetIndexSource(DigitalInput * indexSensor, bool reverse);
	...
	virtual void SetOscilloscopePorts(DigitalOutput * toggle, DigitalOutput *cycle);
private:
	...
	DigitalOutput * m_signalToggle;
	DigitalOutput * m_signalCycle;
	bool m_signalToggleState;
	
	DigitalInput * m_indexSource;
	bool m_shouldReverseIndexSource;
...
protected:
	...
	DISALLOW_COPY_AND_ASSIGN(RAPIDController);
};
